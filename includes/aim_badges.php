...
function check_achievement($achievement_id) {
...
//function check_aimed_achievements($user_id) {

    ////$aimed = get_the_author_meta( 'aimed_badges', $user_id );
    //$achievements = badgeos_get_user_achievements( $user_id );
    //$aimed = update_aimed_achievements( $user_id, $achievements 
    
    

    //foreach ( $achivemenents as $achievement ) {
       //$key = array_search( $achievement, $aimed ); 
       //if ( $key !== false )
           //return false;
    //}
           ////unset( $aimed[$key] );

    ////update_usermeta( $user_id, $aimed );
    ////$aimed = join(" ", $aimed);
